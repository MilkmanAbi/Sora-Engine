...
- **`apps/browser`** — `@sora/browser`, the Sora product: the engine + Sora's config + (later)
  the PaperDesign chrome. Today the renderer is a dev harness, not the real UI.
...
The base is grown deliberately, tested-core-first, before the UI is started. Working + tested:
...
(opt-in) re-encrypts everything. AES-256-GCM, Argon2id via hash-wasm, origin-matched autofill.
...
version is `1.0` and all additions so far have been additive.
